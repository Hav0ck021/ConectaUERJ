...
func (a *authenticationService) UpdatePassword(id string, updatePassword model.UpdatePassword) error {
	...
	newHashedPassword, err := Hash(updatePassword.Password)
	if err != nil {
...
func (a *authenticationService) SendOneTimePassword(email string) error {
	log := slog.With(
		slog.String("func", "SendOneTimePassword"),
		slog.String("service", "authentication"))
...
func (a *authenticationService) CheckOneTimePassword(confirmCodeEmail model.ConfirmCodeEmail) error {
	log := slog.With(
		slog.String("func", "CheckOneTimePassword"),
		slog.String("service", "authentication"))
...
func (a *authenticationService) ForgotPassword(email string) (string, error) {
	log := slog.With(
	...
	token, err := util.GenerateResetPasswordToken(user.Id)
	if err != nil {
		log.Error("error trying create token jwt. Error: ", err)
		return "", model.ErrGenToken
	}
